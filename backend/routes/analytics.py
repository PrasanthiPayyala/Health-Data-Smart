...
def get_all_29_districts(db: Session = Depends(get_db)):
    ...
    live_districts: dict[str, int] = defaultdict(int)
    ...
    max_cases = max(live_districts.values(), default=1)
    result = []
    alias_map = {
    ...
    for display in ALL_29_DISTRICTS:
        upper = display.upper()
        old_match = upper if upper in live_districts else alias_map.get(upper, upper)

        cases = live_districts.get(old_match, 0)
        ...
        intensity = round(cases / max_cases, 2) if max_cases > 0 else 0
        risk_score = round(4.0 + intensity * 5.5, 1) if cases > 0 else 0.0
        ...
        result.append({
            ...
            "status": "live" if cases > 0 else "ready",
        })
    return {"total": 29, "districts": result, "live_count": sum(1 for d in result if d["status"] == "live")}
