...
class StructureCaseRequest(BaseModel):
    transcript: str
    language: str = "en"  # "en", "te", "ur" — affects extraction prompt
...
@router.post("/structure-case")
async def structure_case(req: StructureCaseRequest):
    """
    Audio diary feature: ANM/ASHA speaks 30 sec describing a patient → AI extracts
    structured fields. Input is the transcript (we let the browser do speech-to-text
    via Web Speech API), output is JSON ready to populate the patient form.
    """
    import json
    import re
    ...
    prompt = (
        "You are a clinical data extraction system for the AP Health IQ platform. "
        "An ANM/ASHA field worker has spoken a free-form description of a patient. "
        "Extract structured information and respond with ONLY a valid JSON object — no other text, no markdown.\n\n"
        f"Transcript ({req.language}):\n{req.transcript}\n\n"
        "Required JSON schema:\n"
        "{\n"
        '  "patient_name": "string or null",\n'
        '  "age": integer or null,\n'
        '  "gender": "M" or "F" or null,\n'
        '  "district": "string or null (one of AP\'s 29 districts if mentioned)",\n'
        '  "mandal": "string or null",\n'
        '  "complaints": ["array", "of", "complaints"],\n'
        '  "duration_days": integer or null,\n'
        '  "vitals": {"temperature": float or null, "systole": int or null, "diastole": int or null, "spo2": int or null, "rbs": int or null},\n'
        '  "suggested_diagnosis": "string — your top differential",\n'
        '  "icd10": "ICD-10 code if confident, else null",\n'
        '  "category": "Communicable" or "Non-Communicable" or "Other",\n'
        '  "urgency_level": "low" or "medium" or "high" or "critical",\n'
        '  "recommended_actions": ["array", "of", "next-step actions"],\n'
        '  "confidence": float between 0 and 1\n'
        "}\n\n"
        "Rules:\n"
        "- If a field isn't mentioned in the transcript, use null (don't invent values)\n"
        "- For complaints, normalise to standard medical terms (Fever, Cough, Diarrhoea, etc.)\n"
        "- For urgency: critical = needs immediate referral, high = same-day MO review, medium = routine, low = self-care\n"
        "- Be concise. Single JSON object only. No prose, no commentary, no markdown fences."
    )

    try:
        raw = await ollama_service.chat(prompt)
    except Exception as e:
        raise HTTPException(status_code=503, detail=f"AI extraction failed: {e}")

    # Try to extract JSON even if AI wrapped it in markdown fences or added preamble
    cleaned = raw.strip()
    ...
    if not match:
        raise HTTPException(status_code=500, detail=f"AI did not return JSON. Raw: {raw[:200]}")

    try:
        data = json.loads(match.group(0))
    except json.JSONDecodeError as e:
        raise HTTPException(status_code=500, detail=f"AI returned invalid JSON: {e}. Raw: {raw[:200]}")

    return {
        "structured_case": data,
